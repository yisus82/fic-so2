...
int ejecutar(char *params[],int backgrd, char path[][MAX_PATH],struct proceso espera[],int x,char **env_aux)
{
  	char *ejec;
	int pid;
	int estado;
	...
	if ((pid=fork())==0){
				if (x) { if ((execve(ejec,params,env_aux))==-1) exit(0);}
				  else if ((execv(ejec,params))==-1) exit(0);
			    }
	   else {   
...
int i,j=0,k,l,n;
int estado,pid,pos1,pos2;
char prompt[MAX_PROMPT]="trash-1.02=>";
  ...
  if (args[0]!=NULL)
{
  ...
  if(!(strcmp(args[0],"xenv"))) {
	...
	ejecutar(args_aux,x,path,espera,1,env_aux);
	continue;
  }
  ...
  if (!(strcmp(args[i-2],"&"))) {
				  args[i-2]=NULL;
				  ejecutar(args,1,path,espera,0,env_aux);
				  continue;
		    	       	  }

  ejecutar(args,0,path,espera,0,env_aux);
